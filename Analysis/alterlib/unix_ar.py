# Modified from https://github.com/getninjas/unix_ar to be able to process microsoft .lib files
...
class ArInfo(object):
    ...
    def frombuffer(cls, buffer):
        ...
        mtime = int(mtime, 10)
        uid = int(uid, 10) if uid != b"      " else None
        gid = int(gid, 10) if gid != b"      " else None
        perms = int(perms, 8)
        size = int(size, 10)
        if magic != b'\x60\n':
    ...
    def tobuffer(self):
        ...
        """
        if any(f is None for f in (self._name, self.mtime, self.perms, self.size)):
            raise ValueError("ArInfo object has None fields")

        uidBytes = '      ' if self.uid is None else struct.pack("<6", self.uid).decode('iso-8859-1')
        gidBytes = '      ' if self.gid is None else struct.pack("<6", self.gid).decode('iso-8859-1')

        return (
            '{0: <16}{1: <12}{2}{3}{4: <8o}{5: <10}\x60\n'.format(
                self.name.decode('iso-8859-1'), self.mtime, uidBytes, gidBytes, self.perms, self.size
            ).encode('iso-8859-1')
        )
    ...
    def updatefromdisk(self, path=None):
        ...
        provide them.
        """
        attrs = (self._name, self.size, self.mtime, self.perms)
