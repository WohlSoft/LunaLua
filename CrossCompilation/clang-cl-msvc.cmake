...
function(generate_winsdk_vfs_overlay winsdk_include_dir output_path)
  ...
  foreach(dir ${include_dirs})
    file(GLOB headers RELATIVE "${dir}" "${dir}/*.h" "${dir}/*.rh")
    if(NOT headers)
      continue()
    endif()
...
function(generate_defaultlib_symlinks msvc_lib_dir output_dir)
  execute_process(COMMAND "${CMAKE_COMMAND}" -E make_directory "${output_dir}")
  set(libraries "libcmt" "libcmtd" "msvcrt" "msvcrtd" "oldnames")
  foreach(library ${libraries})
    string(TOUPPER "${library}" symlink_name)
    execute_process(COMMAND "${CMAKE_COMMAND}"
                            -E create_symlink
                            "${msvc_lib_dir}/${library}.lib"
                            "${output_dir}/${symlink_name}.lib")
  endforeach()
endfunction()
...
set(COMPILE_FLAGS
    ...
    -fms-compatibility-version=${MSVC_VER}
    -fms-extensions
    -fms-compatibility
