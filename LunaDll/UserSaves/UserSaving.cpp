...
bool SavedVariableBank::TryLoadWorldVars() {
    ...
    std::wfstream var_file(LunaLua::EncodeUtils::WStr2Str(full_path).c_str(), std::ios::in | std::ios::out);

	// If open failed, try to create empty file
	if(var_file.is_open() == false) {
        var_file.open(LunaLua::EncodeUtils::WStr2Str(full_path).c_str(), std::ios::out);
		var_file.flush();	
		var_file.close();
        var_file.open(LunaLua::EncodeUtils::WStr2Str(full_path).c_str(), std::ios::in | std::ios::out);;
	}
...
void SavedVariableBank::WriteBank() {
    ...
    std::wfstream var_file(LunaLua::EncodeUtils::WStr2Str(full_path).c_str(), std::ios::out | std::ios::trunc);
...
void SavedVariableBank::CheckSaveDeletion() {
	...
	if(GM_STAR_COUNT < GetVar(SPECIAL_SAVE_STR)) {
        ...
        std::wstring full_path = GetSaveFileFullPath(GetSaveFileName());
		ClearBank();
        std::wfstream var_file(LunaLua::EncodeUtils::WStr2Str(full_path).c_str(), std::ios::out);
		InitSaveFile(&var_file);
		var_file.flush();
		var_file.close();
	}
