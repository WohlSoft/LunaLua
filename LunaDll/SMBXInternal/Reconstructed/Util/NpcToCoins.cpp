            ...
            if (!nextNPC->isHidden // (probably isHidden-Field)
                ...
                if (isNotCoinTransformable[id]){ // check our array, if the npc is NOT Transformable
                    if (id == NPCID_GOALTAPE //If it is NOT transformable then check for these special npcs
                        || id == NPCID_FIREBAR
                        || id == NPCID_ROTODISK)
                        nextNPC->unknown_124 = 0; //and set this unknown field to 0
                }
                else{
                    //If it is a transformable npc:

                    nextNPC->momentum.y = nextNPC->momentum.y + 32.0; //now put the npc y pos +32 to offset the coin effect
                    short mask = 0;
                    short npcID = 0;
                    ...
                    native_runEffect((short*)&effectID, &nextNPC->momentum, &animationFrame, &npcID, &mask);
                    ...
                    nextNPC->momentum.height = 0.0;
                    nextNPC->unknown_124 = 0;
                }
            }
        }
        else{
