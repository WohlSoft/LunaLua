...
FFI_EXPORT LunaImageRef* __fastcall FFI_ImageLoad(const char* filename, uint32_t* sizeOut)
{
    ...
    if (!isAbsolutePath(filename)) {
        full_path = getCustomFolderPath() + LunaLua::EncodeUtils::Str2WStr(std::string_view(filename));
    }
    else
    {
        full_path = LunaLua::EncodeUtils::Str2WStr(std::string_view(filename));
    }
...
static void GLPlaceAttributes(std::vector<GLShaderVariableEntry>& out, GLShaderVariableType type, FFI_GL_Draw_Var* vars, unsigned int count)
{
    ...
    {
        auto& unif = vars[i];
        if ((unif.mType == gl::GL_SAMPLER_2D) && (unif.mData != nullptr))
        {
            ...
            out.emplace_back(type, unif.mId, static_cast<gl::GLenum>(unif.mType), unif.mCount, nullptr, data);
        }
        else
        {
            out.emplace_back(type, unif.mId, static_cast<gl::GLenum>(unif.mType), unif.mCount, unif.mData);
        }
    }
}
...
FFI_EXPORT void __fastcall FFI_GLDraw(const FFI_GL_Draw_Cmd* cmd)
{
    ...
    obj->mType = static_cast<gl::GLenum>(cmd->mType);
    obj->mVert = cmd->mVert;
    obj->mTex = cmd->mTex;
...
FFI_EXPORT FFI_ShaderVariableInfo* __fastcall FFI_GetAttributeInfo(FFI_ShaderObjRef* obj, uint32_t idx)
{
    ...
    out.type = static_cast<unsigned int>(info.getType());
...
FFI_EXPORT void __cdecl FFI_SpritePlace(int type, int resNumber, LunaImageRef* img, int xPos, int yPos, const char* extra, int time)
{
    ...
    req.y = yPos;
    req.time = time;
    req.str = extra ? LunaLua::EncodeUtils::Str2WStr(std::string_view(extra)) : L"";
...
FFI_EXPORT void __cdecl FFI_TextDraw(const char* text, int type, int x, int y, double priority, bool sceneCoords)
{
    ...
    std::wstring wText = LunaLua::EncodeUtils::Str2WStr(std::string_view(text));
