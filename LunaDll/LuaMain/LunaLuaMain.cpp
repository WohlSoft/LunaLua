...
void LunaLua::initCodeFiles(lua_State* &L, std::wstring levelPath, std::wstring lapi_path)
{
	TryCloseState(L);
	...
	module(L)
		[
            ...
            def("MusicIsPlaying", (bool(*)())&LuaProxy::MusicIsPlaying),
            def("MusicIsPaused", (bool(*)())&LuaProxy::MusicIsPaused),
            def("MusicIsFading", (bool(*)())&LuaProxy::MusicIsFading),
...

	{
		...
		try
		{
			if(LuaHelper::is_function(L, initFunc)){
				luabind::call_function<void>(L, initFunc, utf8_encode(levelPath), utf8_encode(Level::GetName()));
			}
		}
		catch (error& /*e*/)
		{
...
void LunaLua::initCodeFileWorld(lua_State* &L, std::wstring episodePath, std::wstring lapi_path)
{
	TryCloseState(L);
...

	{
		...
		{
			if(LuaHelper::is_function(L, initFunc)){
				luabind::call_function<void>(L, initFunc, utf8_encode(episodePath));
			}
		}
		catch (error& /*e*/)
		{
...
void LunaLua::DoWorld()
{
	DoCodeWorldFile(mainStateV2);
}
